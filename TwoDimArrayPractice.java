...
public class TwoDimArrayPractice extends JFrame
{
   ...
   public void fillValues() {
	    ...
	    for (int row = 0; row < intArray.length; row++) {
	        ...
	        for (int column = 0; column < intArray[row].length; column++) {
	            // Generate a random number between 50 and 80 (inclusive) and assign it to the array element
	            intArray[row][column] = rand.nextInt(31) + 50;
	            animate(row, column);  // Visualize each value addition (optional)
	        }
	    }
	}
   ...
   */
   public void printArray() {
	    ...
	    for (int row = 0; row < intArray.length; row++) {
	        // Loop through each column in the current row
	        for (int column = 0; column < intArray[row].length; column++) {
	            System.out.print(intArray[row][column] + " ");  // Print each element followed by a space
	            animate(row, column);  // Visualize the printing (optional)
	        }
   ...
   public void setValues( int value, int row )
   {
    ...
    // Write your code here:
	   ...
	   for (int column = 0; column < intArray[row].length; column++) {
	        ...
	        // Note: To animate the algorithm, put this method call as the
	        // last element in your for loop
	        animate(row, column);  // where row is the index of the array's current row
	                               // and column is the index of the array's current column
	    }
   ...
   */
   ...
   // Write your code here:
	   int minimum = Integer.MAX_VALUE;  // Initialize minimum to a large value

	    for (int row = 0; row < intArray.length; row++) {
	        if (intArray[row][column] < minimum) {
	            minimum = intArray[row][column];  // Update minimum if a smaller value is found
	        }
	        animate(row, column, minimum);  // Animate each step
	    }
   ...
   */
   public int countFound( int value )
   {
    ...
    // Write your code here:
	   int num = 0;  // Initialize the count to 0
	    ...
	    for (int row = 0; row < intArray.length; row++) {
	        ...
	        for (int column = 0; column < intArray[row].length; column++) {
	            if (intArray[row][column] == value) {
	                num++;  // Increment the count if the element matches the specified value
	            }
	            // Note: To animate the algorithm, put this method call as the
	            // last element in your inner for loop
	            animate(row, column, num);  // where row is the index of the array's current row,
	                                        // column is the index of the array's current column, and
	                                        // num is the local variable storing the current frequency count
	        }
	    }
